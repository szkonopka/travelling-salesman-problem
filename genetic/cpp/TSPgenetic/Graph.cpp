...
void Graph::readFromTXTFileSym(std::string fileName)
{
...
void Graph::readFromTXTFileAsym(std::string fileName) {
	std::string path = "data/";
	path += fileName;
	std::fstream file;
	file.open(path, std::ios::in);
...
bool Graph::readDataParser(std::string fileName, int format) {
	if (format == 0) {
