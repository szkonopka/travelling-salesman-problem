...
long TSPGenetic::searchForSolution(int popSize, int crossType, int expectedSolution) {
	...
	int tmpDist, globMutation = 0, i, diversificationUseless = 0;
	for (i = 0; i < 1000; i++) {
		tmpDist = bestDist;
...
void TSPGenetic::insert(int *arr, int a, int b) {
...
void TSPGenetic::genNewGeneration(int popSize, int crossType) {
	...
	for (int i = 0; i < popSize; i++) {
		...
		if (dis(gen) < selectFromNewPopProb) {
			for (int j = 0; j < g_size; j++) {
